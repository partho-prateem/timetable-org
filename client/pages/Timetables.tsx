...
import { Session, TimetableGrid, getConflictPairs, detectConflicts } from "@/components/schedule/TimetableGrid";
...
function findDifferentRoom(items: Session[], forSession: Session) {
  ...
  const occupied = new Set(items.filter(it=>it.id!==forSession.id).map(it=>`${it.day}:${it.hour}:${it.room}`));
...
export default function Timetables() {
    ...
    for (const p of pairs) {
      ...
      if (p.reasons.includes("room")) {
        ...
        if (newRoom) {
          ...
          suggestions.push({ id: b.id, from: { day: b.day, hour: b.hour }, to: { day: b.day, hour: b.hour }, reason: "change room to free room" });
          continue;
        }
      }
      // else prefer moving the one with fewer conflicts
      const aConfCount = pairs.filter(pp=>pp.aId===a.id||pp.bId===a.id).length;
      const bConfCount = pairs.filter(pp=>pp.aId===b.id||pp.bId===b.id).length;
      if (aConfCount <= bConfCount) target = a; else target = b;
      const next = findNextFreeSlot(items, target);
      suggestions.push({ id: target.id, from: { day: target.day, hour: target.hour }, to: { day: next.day, hour: next.hour }, reason: `move to reduce conflicts (${p.reasons.join(",")})` });
    }
  ...
  const applySuggestion = (s: Suggestion) => {
    ...
    if (s.from.day===s.to.day && s.from.hour===s.to.hour && s.reason.includes('room')) {
  ...
  return (
    <AppLayout>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <Button variant="secondary" onClick={autoResolve} disabled={conflictSet.size===0}>Auto-resolve conflicts</Button>
        {conflictSet.size>0 && <div className="ml-auto text-sm text-red-600">{conflictSet.size} conflict(s)</div>}
      </div>
      <ConflictPanel suggestions={suggestions} onApply={applySuggestion} onApplyAll={applyAll} />
